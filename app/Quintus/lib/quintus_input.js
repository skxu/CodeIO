...
Quintus.Input = function(Q) {
  var KEY_NAMES = { LEFT: 37, RIGHT: 39, SPACE: 32, TAB: 9,
  ...
  Q.component("platformerControls", {
    ...
    step: function(dt) {
      ...
      if(p.totalTime >= p.jumpDelay) {
        p.landed = 0.2;
        p.startPausing = false;
        p.totalTime = 0;
        p.maxJumps = 0;
      }
      if(Q.inputs['left']) {
      ...
      if((p.maxJumps >= 0) && (p.landed > 0 && (Q.inputs['up'] || Q.inputs['action']))) {
        p.vy = p.jumpSpeed;
        p.landed = -dt;
      }

      if(Q.input_release['up']) {
        if(p.maxJumps > 0 && Q.inputs['up']) {
          p.vy = p.jumpSpeed;
          p.maxJumps -= 1;
